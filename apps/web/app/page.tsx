...
function RankingWidget() {
  const m = LIVE_MATCH;
  const posColors = ["text-brand-gold", "text-slate-400", "text-amber-700", "text-brand-muted", "text-brand-muted"];
...
function MatchCard({ torneo, variant }: { torneo: TorneoCard; variant: "abierto" | "proximo" | "finalizado" }) {
  return (
...
function FilterChips() {
  const filters = ["Todos", "\uD83C\uDFC6 Champions", "\u26BD Liga 1", "\uD83C\uDF0E Copa Lib.", "\uD83C\uDFF4\uDB40\uDC67\uDB40\uDC62\uDB40\uDC65\uDB40\uDC6E\uDB40\uDC67\uDB40\uDC7F Premier"];
  return (
...
export default function HomePage() {
  ...
  const handleBottomNav = (tab: string) => {
    ...
    // mis-lukas y perfil requieren login (Sprint 1)
  };
