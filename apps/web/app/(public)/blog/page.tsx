    ...
    <div className="mx-auto max-w-[1200px] px-4 py-10 md:px-6 md:py-14">
      <header className="mb-8">
        ...
        <p className="text-[16px] leading-[1.7] text-body">
          Análisis editorial de Habla! sobre la Liga 1, torneos
          internacionales, predicciones deportivas y operadores de apuestas
  ...
  return (
    ...
    >
      <div className="flex flex-1 flex-col p-5">
        ...
        <div className="mt-4 flex items-center justify-between text-[11px] text-muted-d">
          <span>{author}</span>
    ...
    >
      {page > 1 ? (
        <Link
          ...
          className="rounded-sm border border-light bg-card px-4 py-2 font-bold text-dark hover:bg-subtle"
        >
          ← Anterior
        </Link>
      ) : (
        <span className="rounded-sm border border-light bg-subtle px-4 py-2 font-bold text-soft">
          ← Anterior
        </span>
      )}
      <span className="font-mono text-muted-d">
        {page} / {totalPages}
      </span>
      {page < totalPages ? (
        <Link
