...
> Última actualización: 19 de Abril 2026 (Hotfix #8 post-Hotfix #7: reloj local del minuto del partido — el `LiveHero` ya no se queda pegado en `⏱ —` o en el último valor del poller (30s) entre ticks; nuevo hook `useMinutoEnVivo` corre un interval de 1s que proyecta `elapsed` local anclado al `snapshotUpdatedAt` del cache del server para statuses avanzables (1H/2H/ET) con cap 45/90/120; `RankingUpdatePayload` + endpoints REST `/api/v1/torneos/:id/ranking` y `/api/v1/live/matches` extendidos con `statusShort` + `snapshotUpdatedAt`; estados fijos (HT/FT/PEN) delegan al mapper `formatMinutoLabel` sin interval; auto-reconciliación defensiva en `finalizarTorneo` — si detecta torneo ya FINALIZADO pero con tickets premiados sin `TransaccionLukas PREMIO_TORNEO` suficiente, dispara `reconciliarTorneoFinalizado` automáticamente con logger.warn + try/catch para que el poller no falle — repara los 2 torneos pre-deploy del Hotfix #7 y cualquier regresión futura sin requerir admin manual; nuevo helper `detectarCreditoIncompleto(torneoId)` compara `sum(Ticket.premioLukas)` vs `sum(TransaccionLukas.monto WHERE tipo=PREMIO_TORNEO AND refId=torneoId)`)
...
**Bug #22 — `⏱ —` congelado o pegado al último valor del poller en `/live-match`.** El PO reportó: partido Cienciano vs UCV Moquegua en vivo, ~10 minutos jugados, el hero mostraba `⏱ —` al entrar a la página y no cambiaba. Root cause: el poller corre cada 30s y actualiza el cache in-memory con `{statusShort, elapsed, updatedAt}` de api-football; el `LiveHero` recibía `minutoLabel` server-rendered ya computado por `formatMinutoLabel` y simplemente lo pintaba. Entre ticks del poller (30s) el label era el mismo string — si entraste justo después de un tick, veías el mismo número durante 30 segundos. Peor aún: si el proceso reinició hace poco, el cache estaba vacío y el primer render mostraba `—` hasta el próximo tick (hasta 30s). El Hotfix #6 Ítem 3 ya había extendido el TTL a 30 min y agregado polling REST cada 45s, pero eso no resuelve la UX entre ticks. La expectativa del usuario es un reloj de estadio que avance segundo a segundo. Fix en 7 piezas:

- **Pieza 1 — Hook puro `useMinutoEnVivo`.** Nuevo archivo `apps/web/hooks/useMinutoEnVivo.ts` con la función exportada `computeElapsedLocal(input)` (pura, testeable) + el hook `useMinutoEnVivo(input)`. El hook acepta `{ statusShort, elapsed, snapshotUpdatedAt, fallbackLabel }`, tiene `setInterval(1s)` que re-computa el minuto localmente cuando `statusShort ∈ {1H, 2H, ET}` (statuses "avanzables"). En HT/FT/NS/PEN/P/SUSP/ABD/CANC/AWD/WO el label es fijo y el hook delega al `formatMinutoLabel` sin correr el interval. Fórmula: `elapsedLocal = elapsed + Math.floor((now - snapshotUpdatedAt) / 60000)` con cap por status (45 en 1H, 90 en 2H, 120 en ET). Si el server reporta `elapsed` ya superando el cap (ej. 2H con `elapsed=94` por descuento largo del PT), el hook respeta el server — el cap solo aplica a la extensión LOCAL. Reset al cambiar snapshot (dependencies: `[statusShort, elapsed, snapshotUpdatedAt, esAvanzable]`).
- **Pieza 2 — `RankingUpdatePayload` extendido (aditivo).** `apps/web/lib/realtime/events.ts` agrega `statusShort: string | null` y `snapshotUpdatedAt: number | null`. El cliente WS los recibe en cada tick del poller y usa `snapshotUpdatedAt` como ancla del reloj local. No tocamos `minutoPartido` ni `minutoLabel` — siguen siendo válidos y son fallback para estados fijos.
- **Pieza 3 — `emitirRankingUpdate` los lee del cache.** `emitters.ts` extrae `snapshot?.statusShort ?? null` y `snapshot?.updatedAt ?? null` junto al label ya existente. No requiere cambios en el poller — ya llama `setLiveStatus` cada tick.
- **Pieza 4 — Endpoint REST `/api/v1/torneos/:id/ranking`.** Agrega ambos campos al response. El polling de fallback del Hotfix #6 Ítem 3 sigue funcionando igual; ahora además re-ancla el reloj local si el WS se desconecta.
- **Pieza 5 — Endpoint `/api/v1/live/matches`.** Mismo cambio aditivo en el sidebar widget. No cambia el consumidor actual del sidebar (el widget solo muestra score + minuto label), pero deja la puerta abierta para que en un futuro el widget use el hook.
- **Pieza 6 — SSR de `/live-match`.** `buildLiveTabs` y `tryBuildFinalizedTab` en `app/(main)/live-match/page.tsx` ahora asignan `statusShort`, `elapsed` y `snapshotUpdatedAt` al `LiveMatchTab` desde el snapshot del cache. Para partidos FINALIZADOS el status es FT/AET/PEN, no-avanzable, así que el hook no corre el interval.
- **Pieza 7 — `LiveHero` es client component.** El archivo pasó a `"use client"`. Recibe `minutoLabel` (fallback SSR) + los 3 nuevos campos `statusShort/elapsed/snapshotUpdatedAt` y llama `useMinutoEnVivo` para derivar el label mostrado. `LiveMatchView` propaga con preferencia WS → SSR: `statusShort={live.statusShort ?? active.statusShort}`, `elapsed={live.minutoPartido ?? active.elapsed}`, `snapshotUpdatedAt={live.snapshotUpdatedAt ?? active.snapshotUpdatedAt}`. El `useRankingEnVivo` hook expone ambos nuevos campos y preserva null entre polls REST (igual pattern que el Hotfix #6 Ítem 3 con `minutoLabel`) pero los pisa crudos al llegar un WS (el WS siempre trae snapshot fresco).

**Alternativa descartada:** subir la frecuencia del poller a 10s para "suavizar" el label. Se descartó — consume cuota de api-football (150 reqs/día plan gratis; 30s es sweet spot para 20 partidos concurrentes) y no resuelve el caso del cache vacío post-restart ni el primer render antes del WS. El reloj local cuesta un `setInterval(1s)` por instancia de `LiveHero` — hay máximo 1 activa por página — y se desconecta correctamente al unmount.
...
**Tests agregados:**
- `tests/use-minuto-en-vivo.test.ts` (42 casos): matriz de `computeElapsedLocal` puro (1H/2H/ET avanza, HT/FT/NS/PEN no, cap por status, server ya superó cap, clock skew, null-inputs) + AST sobre `useMinutoEnVivo.ts` (use client, interval 1s, cleanup, deps del effect, CAPs numéricos, STATUSES_AVANZANDO, import de formatMinutoLabel) + wiring end-to-end (`RankingUpdatePayload` con los 2 nuevos campos, `emitters.ts` los lee del snapshot, endpoints REST los exponen, `LiveHero` es client + usa `useMinutoEnVivo` sin `renderMinutoLabel` directo, `LiveMatchView` propaga con fallback SSR, `useRankingEnVivo.RankingSnapshot` declara los 2 campos, SSR de `/live-match/page.tsx` los asigna al `LiveMatchTab`).
...
- **Minuto del partido en vivo se deriva con `useMinutoEnVivo` — reloj local anclado al snapshot del server:** el hook `apps/web/hooks/useMinutoEnVivo.ts` es el único punto donde la UI decide qué label mostrar para el minuto en el `LiveHero`. Toma `{ statusShort, elapsed, snapshotUpdatedAt, fallbackLabel }` y devuelve el string. Para statuses avanzables (`1H`, `2H`, `ET`), corre un `setInterval(1s)` que proyecta `elapsed + Math.floor((Date.now() - snapshotUpdatedAt) / 60000)` capped a 45/90/120 respectivamente. Para estados fijos (`HT`, `FT`, `NS`, `PEN`, `P`, `SUSP`, `ABD`, `CANC`, `AWD`, `WO`, `PST`, `INT`, `BT`, `TBD`, `AET`) delega al mapper `formatMinutoLabel` sin interval (ningún consumo de CPU cuando el partido no avanza). Al cambiar el snapshot (nuevo `snapshotUpdatedAt`), el hook resetea el cómputo. Prohibido renderizar el minuto directamente con `renderMinutoLabel` o con lógica inline — siempre vía el hook. Test de regresión en `tests/live-hero-minute.test.ts` verifica que `LiveHero.tsx` importe `useMinutoEnVivo` y no tenga `renderMinutoLabel` directo. Razón: el Bug #22 del Hotfix #8 — el label server-rendered quedaba pegado entre ticks del poller (30s), el usuario esperaba un reloj de estadio que avance segundo a segundo; también cubre el caso del cache vacío post-restart (el hook muestra el fallback SSR hasta que llegue el primer WS/REST con snapshot).

- **`RankingUpdatePayload` y endpoints REST `/api/v1/torneos/:id/ranking` y `/api/v1/live/matches` incluyen `statusShort: string | null` + `snapshotUpdatedAt: number | null`:** son los 2 inputs que alimentan el reloj local del hook `useMinutoEnVivo`. El `statusShort` viene directo de `fixture.status.short` de api-football (el poller lo captura en `setLiveStatus`); `snapshotUpdatedAt` es `Date.now()` del momento en que el poller escribió al cache. El cliente usa ambos para anclar su reloj local al tiempo del server — si el proceso del cliente tiene clock skew pequeño respecto al server (latencia de red), el reloj local se auto-corrige en el próximo tick del poller porque el `snapshotUpdatedAt` se re-ancla. El `useRankingEnVivo` hook preserva null entre polls REST (igual pattern que `minutoLabel` del Hotfix #6 Ítem 3) para no perder el ancla cuando el cache está stale; el WS siempre trae snapshot fresco y pisa crudos. Razón: el Bug #22 del Hotfix #8 — sin `snapshotUpdatedAt` no hay manera de saber cuánto tiempo pasó desde que el server capturó el `elapsed`, y el hook no puede proyectar.
