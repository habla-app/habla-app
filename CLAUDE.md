...
> Última actualización: 19 de Abril 2026 (Hotfix #4 post-Sub-Sprint 5: ComboModal con 6 status states y feedback panel post-submit; balance global se sincroniza en header + /mis-combinadas tras inscripción vía `useLukasStore.setBalance`; `/live-match` **excluye** partidos con todos los torneos cancelados — revert del Hotfix #3 por revisión del PO; minuto del partido ahora se renderiza como label legible ("23'", "ENT", "FIN", "Prór. 95'") mapeado por `formatMinutoLabel` en lugar de "?"; switcher del /live-match solo muestra EN_VIVO con sección separada de "Partidos finalizados de hoy"; filter chips por liga en /live-match con lista dinámica derivada de partidos EN_VIVO actuales)
