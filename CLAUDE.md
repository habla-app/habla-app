...
El binario `pg_dump` debe ser **versión >= servidor**. Si Railway corre Postgres 18 y el cliente es 16, pg_dump revienta con `aborting because of server version mismatch`. Para que el repo de Alpine traiga `postgresql18-client` hace falta Alpine **>= 3.22**, así que el Dockerfile pinea `node:20-alpine3.22` (no `node:20-alpine` mutable, que puede resolverse a 3.20/3.21 sin paquete 18 en repo estable). Cuando Railway pase a Postgres 19/20 hay que (a) bumpear el pin de Alpine al primer release que tenga `postgresql<N>-client`, y (b) bumpear el número del paquete `apk add`. Diagnóstico: el `errorMsg` del último row de `BackupLog` (vía `GET /api/v1/admin/backup/historial`) muestra el mismatch exacto.
