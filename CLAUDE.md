...
> Última actualización: 1 May 2026 — Lote H cerrado (Microcopy + emails + WhatsApp templates + sistema de notificaciones in-app: catálogo central de microcopy en `apps/web/lib/copy/` con 10 archivos i18n-ready (`auth.ts` · `home.ts` · `partidos.ts` · `comunidad.ts` · `premium.ts` · `perfil.ts` · `admin.ts` · `errores.ts` · `empty-states.ts` · `notificaciones.ts` con `TOAST_COPY` + `BANNER_COPY`) + helper `tpl(template, vars)` para `{variable}` + barrel `index.ts`. Templates React Email en `apps/web/lib/email/`: 4 componentes base reusables (`_components/Layout` con `<Html>/<Head>/<Preview>/<Body>` Container 600px max-width Outlook-safe + `Header` con logo cuadrado 120×120 + `Footer` obligatorio con apuesta responsable+Tugar 0800-19009+links preferencias/eliminar/dominio + `Button` como `<a>` con padding por variante gold/outline/whatsapp) + 11 templates canónicos (`MagicLinkAuth` · `BienvenidaRegistro` · `ConfirmacionNewsletter` · `BienvenidaPremium` · `FacturaPremium` · `RenovacionRecordatorio` · `FalloPagoPremium` · `ReembolsoConfirmado` · `PremioMensualSolicitarDatos` · `PremioMensualPagado` · `EliminacionConfirmada`). Helper `sendEmail(opts)` con `render()` de `@react-email/components` + retry 3x backoff (1s/2s/4s) reusando `enviarEmail` del Lote E (fail-soft sin RESEND_API_KEY) + 5 from-addresses por categoría (`auth@`/`hola@`/`newsletter@`/`premium@`/`legal@`). Helpers de formato compartidos `fmtFechaLarga`/`fmtFechaCorta`/`fmtSolesCentimos`/`fmtSolesNumero`/`ordinalEs`. WhatsApp templates en `apps/web/lib/services/whatsapp/templates.ts`: 7 templates HSM tipados (`factura_premium`+`fallo_pago_premium`+`renovacion_recordatorio`+`cancelacion_efectiva`+`reembolso_confirmado`+`premio_mensual_listo`+`welcome_bot_inicial`) con `{{N}}` en orden, `bodyTemplate` literal para preview, helper `enviarTemplate<K>(key, to, vars)` 100% type-safe + `puedeEnviarMarketingAhora()` que enforce 9 AM-8 PM Lima para categoría MARKETING + redactPhone para logs sin PII. Método `enviarTemplate` agregado a `WhatsAppBusinessClient` (REST a graph.facebook.com con timeout 30s + parser JSON + manejo 429 rate-limit). Sistema de notificaciones in-app con `sonner@2`: `<Toaster>` cliente posición bottom-right max 3 visibles closeButton richColors (`components/ui/Toaster.tsx`), helper `showToast.{success,info,warning,error,persistent,dismiss,promise}` (`lib/toast.ts`) con duraciones según severidad (3s/4s/5s/6s) + tracking `toast_visto`/`toast_accion_clickeada` fire-and-forget, `<Banner>` componente persistente con 3 variantes info/warning/error usando tokens del Lote A (alert-info-bg/alert-warning-bg/alert-danger-bg) + accion opcional Link/onClick + dismissible (`components/ui/Banner.tsx`). Backward-compat 100%: `<ToastProvider>` y `useToast()` legacy en `Toast.tsx` reescritos como wrappers finos sobre sonner — los 16+ call sites existentes (`PrediccionForm`, `NewsletterAdminPanel`, `PicksPremiumView`, `SuscripcionDetalleView`, `ChannelWhatsAppView`, etc.) siguen funcionando con `toast.show("✅ ...")` que detecta el prefix y mapea a la severidad correcta. Cero migraciones de BD. Reglas v3.1 cubiertas: cero promesas legales (no "ganarás", no "garantizamos rentabilidad" en NINGÚN copy), apuesta responsable + Línea Tugar mencionados en TODO email Premium + footer obligatorio en TODOS los emails, MARKETING fuera de horario rechazado server-side, watermark email no aplica a templates HSM (sí a free-form picks Lote E), idempotencia y rate-limit ya cubiertos por el cliente WhatsApp del Lote E. Eventos analíticos nuevos: `toast_visto`, `toast_accion_clickeada`, `wa_template_enviado`, `wa_template_falla_envio`. Deps añadidas a `apps/web/package.json`: `sonner@^2.0.7` + `@react-email/components@^1.0.12` (runtime) + `react-email@^6.0.5` (devDep, CLI `react-email dev` para preview local opcional).
...
**Ruta crítica:** A ✅ → B ✅ → C ✅ → E ✅ → D ✅ → F ✅ → G ✅ → H ✅ → J. I (mobile audit + PWA) puede paralelo a J.
