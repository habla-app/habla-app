...
> Última actualización: 30 Apr 2026 — Lote C cerrado (capa autenticada mobile-first: nueva URL `/comunidad/torneo/[slug]` con form de 5 mercados + pick Premium inline + affiliate inline + leaderboard preview, /perfil rediseñado con PremiumStatusCard + MisCasasConectadas + ReferidoModal, /mis-combinadas → /mis-predicciones con StatsHero + EvolucionChart SVG + 5 tabs, /comunidad con MisStatsMini + PremiosMensualesCard + MesesCerradosLink, /live-match con AlertasPremium + cross-link C→B, /comunidad/[username] y /comunidad/mes/[mes] mobile-first, redirects 301 para /torneo/:id legacy via Server Component + /mis-combinadas + /torneos en next.config).
