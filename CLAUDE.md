...
- **Lote 6C-fix3 — Sistema de auditoría completo de balances (26 Abr 2026):** Decisión post-mortem del bug 6C-fix2: blindar el sistema antes de seguir tocando UI. Implementación de [`lib/services/auditoria-balances.service.ts`](apps/web/lib/services/auditoria-balances.service.ts) que evalúa **13 invariantes** del sistema de 3 bolsas para TODOS los usuarios y torneos en una sola operación: I1 `balanceLukas == suma bolsas`, I2 `balanceLukas == sum(tx)`, I3-I5 cada bolsa == sum(tx por bolsa), I6 `balanceCompradas == sum(saldoVivo vigente)`, I7 compras vencidas con `saldoVivo > 0`, I8 balances no negativos, I9 ENTRADA_TORNEO con `metadata.composicion` válida, I10 todo `Ticket` con `ENTRADA_TORNEO` asociada, I11 pozoBruto == suma entradas, I12 torneo CANCELADO reembolsos == pozoBruto, I13 torneo FINALIZADO premios == pozoNeto. **Eficiencia**: usa `groupBy` agregado de Prisma (10 queries totales, no N+1) — para MVP-scale (~10k tx) corre en <1s. Solo lectura, nunca muta. Endpoints: `GET /api/v1/admin/auditoria/full` (scan masivo, top 100 hallazgos) + `GET /api/v1/admin/auditoria/usuario/[id]` (drill-down con transacciones cronológicas + agregados por tipo/bolsa + reconstrucción de la historia). Ambos guard `Bearer CRON_SECRET`. Reporte estructurado por invariante con counts ok/fallidos, hallazgos ordenados por severidad. Detecta categorías que el endpoint legacy `/auditoria/balance` (Lote 6B-fix2) NO veía: divergencia tx vs balance, FIFO de saldoVivo roto, vencimientos no aplicados, integridad de torneos, tickets huérfanos. `tsc + lint` sin errores.
- **Lote 6C-fix3 completo — 4 fases de blindaje (26 Abr 2026):** Tras detectar bug histórico en producción (3 usuarios con bonus mal categorizado en `balanceCompradas` por backfills antiguos), se implementaron 4 fases:
  - **Fase 1 — Recategorización one-shot.** `POST /api/v1/admin/auditoria/recategorizar-bolsas` (Bearer CRON_SECRET). Para cada usuario con I1+I2 OK, redistribuye internamente: `balanceBonus = sum(tx BONUS)`, `balanceGanadas = sum(tx GANADAS)`, `balanceCompradas = balanceLukas - bonus - ganadas` (residual). `balanceLukas` total NO cambia. Idempotente: en una segunda corrida, los deltas son 0. Skippea usuarios con I1 violada (requieren `/balance/corregir` primero) o que darían bolsas negativas (análisis manual). Crea `TransaccionLukas AJUSTE` por usuario recategorizado.
  - **Fase 2 — Cron diario con alerta por email.** Job G en [`instrumentation.ts`](apps/web/instrumentation.ts) que tick cada 1h, skip si corrió <23h. Llama `auditarTodos`, si hay hallazgos `severity: error` envía email al admin (`ADMIN_ALERT_EMAIL`) con el reporte agregado. Solo `errors` disparan email; `warns` solo log. Template HTML `auditoriaAlertaTemplate` en [`lib/emails/templates.ts`](apps/web/lib/emails/templates.ts) con tabla de invariantes fallidas + top 20 hallazgos. Wrapper `enviarAlertaAuditoria` en [`notificaciones.service.ts`](apps/web/lib/services/notificaciones.service.ts) — si `ADMIN_ALERT_EMAIL` no está seteado, log warn y no envía (no rompe cron).
  - **Fase 3 — Guards de consistencia post-mutación en 5 services.** Helper compartido `verificarConsistenciaBalance(tx, usuarioId, contexto)` en [`lib/services/balance-consistency.helper.ts`](apps/web/lib/services/balance-consistency.helper.ts). Lee usuario refrescado tras UPDATE, si `balanceLukas != suma bolsas` loggea `error`. Aplicado al final de cada `prisma.$transaction` en: `canjes.service` (solicitar + cancelar), `ranking.service` (finalizarTorneo + reconciliar), `compras.service` (acreditar), `vencimiento-lukas.job` (vencer). El sexto punto (`torneos.service.descontarEntrada`) ya tenía guard inline desde Lote 6C-fix2 — patrón documentado para futuros services.
  - **Fase 4 — Tests AST de invariantes.** [`apps/web/tests/invariantes-balance.test.ts`](apps/web/tests/invariantes-balance.test.ts) con 17 tests que verifican via regex que: cada service importa el helper, lo invoca con su contexto, las 13 invariantes están definidas en el service de auditoría, los 3 endpoints admin están guard por CRON_SECRET, y el Job G está cableado correctamente. `pnpm test invariantes-balance` corre los 17 en <10ms.
  - Nueva env var: `ADMIN_ALERT_EMAIL=hablaplay@gmail.com` (o el destinatario que prefiera el operador). Si falta, las alertas se loggean pero no se envían.
...
- `GET /api/v1/admin/auditoria/full` — Lote 6C-fix3. Scan masivo de las 13 invariantes del sistema de balances. Devuelve reporte agregado + top 100 hallazgos ordenados por severidad. Solo lectura. Auth `Bearer <CRON_SECRET>`. Costo aprox <1s para MVP-scale.
- `GET /api/v1/admin/auditoria/usuario/[id]` — Lote 6C-fix3. Drill-down per-user: estado de los 4 balances, hallazgos que afectan al usuario, agregados por tipo/bolsa, reconstrucción a partir de transacciones, slice cronológico de las últimas 200 transacciones. Auth `Bearer <CRON_SECRET>`.
- `GET /api/v1/admin/auditoria/balance` (legacy, Lote 6B-fix2) — solo verifica I1 (`balanceLukas == suma bolsas`). Mantenido por compatibilidad pero el scan completo arriba es más exhaustivo. Auth `Bearer <CRON_SECRET>`.
- `POST /api/v1/admin/auditoria/recategorizar-bolsas` — Lote 6C-fix3 Fase 1. Corrige bonus históricos mal categorizados como `balanceCompradas`. Solo recategoriza usuarios con I1+I2 OK (caso contrario skip). Idempotente. `balanceLukas` total no cambia, solo redistribución interna. Crea `TransaccionLukas AJUSTE` por usuario tocado. Auth `Bearer <CRON_SECRET>`.
- `POST /api/v1/admin/auditoria/reset-y-inyectar-bonus` — Lote 6C-fix3. Endpoint de uso EXCEPCIONAL (testing pre-producción). Body: `{ confirmacion: "INYECTAR_TEST_LUKAS", montoBonus?: 100 }`. Para cada usuario activo: borra todas sus `TransaccionLukas AJUSTE` (ruido de fixes anteriores), recalcula bolsas a partir de tx legítimas, si `sumTx COMPRADAS < 0` genera BONUS compensador (caso bonus pre-Lote 6A mal categorizado), inyecta `montoBonus` Lukas a la bolsa BONUS, persiste los 4 balances coherentes. NO IDEMPOTENTE — corriéndolo dos veces inyecta 2x. Auth `Bearer <CRON_SECRET>`.
