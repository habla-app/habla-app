...
**Ruta crítica v3.2:** **K ✅** → **L ✅** → **M ✅** → **Q ✅** → **R ✅** → N → O → P. K es prerequisito de todo (schema, URLs, paywall config, AuthGate). L es prerequisito de M, N, O, P (datos del motor alimentan vistas). M deja productivas las dos vistas centrales del usuario (Las Fijas + Liga); Q reescribe esas 4 vistas con fidelidad 1:1 al mockup HTML; **R completa la portación del CSS del mockup — `mockup-styles.css` ahora cubre TODAS las vistas del mockup (públicas + admin), los lotes N + O + P solo portan HTML→JSX sin escribir CSS nuevo**; N completa Home + Socios + Reviews + Perfiles aplicando el mismo enfoque; O y P aterrizan admin operación + analítica.
