...
- Endpoints de auditoría de balances (Lote 6C-fix3) — listados con detalle en la entrada del lote arriba. Todos `Bearer <CRON_SECRET>`. Resumen: `GET /admin/auditoria/full` (scan), `GET /admin/auditoria/usuario/[id]` (drill-down), `GET /admin/auditoria/balance` (legacy I1 only), `POST /admin/auditoria/balance/corregir`, `POST /admin/auditoria/recategorizar-bolsas`, `POST /admin/auditoria/reset-y-inyectar-bonus`, `POST /admin/auditoria/mover-compradas-a-bonus`.
