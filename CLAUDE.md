...
> Última actualización: 19 de Abril 2026 (Hotfix #3 post-Sub-Sprint 5: ComboModal se renderiza via React portal a `document.body` para que no quede atrapado en el stacking context del MatchCard — corregido "modal parpadea y página congela" post-merge del Hotfix #2; `/live-match` tolera partidos cuyos torneos están todos CANCELADO mostrando score + eventos con cartel "sin torneo activo")
...
- **`/live-match` tolera partidos sin torneo activo:** un partido EN_VIVO puede tener todos sus torneos CANCELADO (regla: `<2 inscritos` al cierre → cancelar + reembolsar). El helper `obtenerLiveMatches` incluye todos los torneos en `include.torneos` (incluso CANCELADO) y `elegirTorneoPrincipal` retorna `null` si todos están cancelados. El frontend (`LiveMatchView`) rendera el hero + eventos + stats del partido igual, pero muestra `<SinTorneoActivo />` (card con copy "🏟️ Este partido no tiene torneo activo") en el tab Ranking en lugar del ranking real. Deep-linking por `?partidoId=<id>` permite volver a ese tab específicamente. Razón: el Bug #5 del Hotfix #3 — Manchester City vs Arsenal en prod tenía torneos todos cancelados y el filtro previo lo escondía con empty state global.
