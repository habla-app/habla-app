...
> Última actualización: 19 de Abril 2026 (Hotfix #8 post-Hotfix #7: reloj local del minuto del partido — el `LiveHero` ya no se queda pegado en `⏱ —` o en el último valor del poller (30s) entre ticks; nuevo hook `useMinutoEnVivo` corre un interval de 1s que, para la fase 1H, proyecta el minuto ANCLADO a `Partido.fechaInicio` (persistido en BD desde el import del fixture — disponible desde el primer render, aun con cache in-memory vacío post-restart); para 2H/ET usa el `elapsed` del server como ancla con tracking local del `elapsedAnchorAt` (useRef que resetea al cambiar elapsed/statusShort); fases fijas (HT/FT/PEN/NS/SUSP/ABD/CANC/AWD) delegan al mapper `formatMinutoLabel` sin interval; heurística de fallback basada solo en fechaInicio cuando el server aún no envió statusShort (muestra 1T→ENT→2T→"—" según tiempo transcurrido); `RankingUpdatePayload` + endpoints REST extendidos con `statusShort` (aditivo — `snapshotUpdatedAt` fue descartado en favor de `fechaInicio` que ya vive en BD, refactor post-feedback del PO sobre complejidad); auto-reconciliación defensiva en `finalizarTorneo` — si detecta torneo ya FINALIZADO pero con tickets premiados sin `TransaccionLukas PREMIO_TORNEO` suficiente, dispara `reconciliarTorneoFinalizado` automáticamente con logger.warn + try/catch para que el poller no falle — repara los 2 torneos pre-deploy del Hotfix #7 y cualquier regresión futura sin requerir admin manual; nuevo helper `detectarCreditoIncompleto(torneoId)` compara `sum(Ticket.premioLukas)` vs `sum(TransaccionLukas.monto WHERE tipo=PREMIO_TORNEO AND refId=torneoId)`)
...
**Trade-off conocido:** si el partido arranca DEMORADO (kickoff real ≠ `fechaInicio` programado), la heurística del 1H sobre-cuenta el minuto hasta que llegue el primer tick del poller con `elapsed` real del API. Mitigamos con `max(elapsed, mFromKickoff)` — cuando el server reporta 10 pero la heurística dice 30, mostramos 30 clamped a 45 por el cap. El usuario ve un valor razonable; al próximo tick del poller el `elapsed` server "gana" y el reloj se re-ancla (useRef detecta que elapsed cambió).
...
- **Minuto del partido en vivo se deriva con `useMinutoEnVivo` — reloj local anclado a `Partido.fechaInicio` (persistido en BD):** el hook `apps/web/hooks/useMinutoEnVivo.ts` es el único punto donde la UI decide qué label mostrar para el minuto en el `LiveHero`. Toma `{ fechaInicio, statusShort, elapsed }` y devuelve el string. Lógica por fase:
  - **1H**: ancla a `fechaInicio`. Minuto = `max(1, min(45, floor((Date.now() - fechaInicio)/60000) + 1))`. Si hay `elapsed` server, toma `max(elapsed, mFromKickoff)` para tolerar partidos que arrancaron tarde. Si server reporta `elapsed > 45` (descuento largo), respeta sin clampear.
  - **2H / ET**: ancla al `elapsed` del server (descanso HT variable). El hook trackea en `useRef<number>` (`elapsedAnchorAt`) cuándo recibió el último `elapsed` y proyecta `elapsed + floor((Date.now() - elapsedAnchorAt)/60000)` con cap 90/120.
  ...
  - **Sin `statusShort` (cache vacío post-restart)**: heurística basada solo en `fechaInicio` (<0 min → "Por empezar", 0-45 → minuto estimado, 45-60 → "ENT", 60-105 → minuto estimado del 2T con HT de 15 min, >105 → "—"). Se corrige en cuanto llegue el primer tick del poller.

  Prohibido renderizar el minuto con `renderMinutoLabel` directo o con lógica inline — siempre vía el hook. Test de regresión en `tests/live-hero-minute.test.ts` verifica que `LiveHero.tsx` importe `useMinutoEnVivo` y no tenga `renderMinutoLabel` directo. Razón: el Bug #22 del Hotfix #8 — el label server-rendered quedaba pegado entre ticks del poller (30s), y si el cache estaba vacío (post-restart) el hero mostraba "—" durante hasta 30s. Anclar a `fechaInicio` (estable en BD) en vez de `snapshotUpdatedAt` del cache in-memory elimina la dependencia del cache para el primer render.
