...
> Última actualización: 19 de Abril 2026 (Hotfix #7 post-Hotfix #6: `finalizarTorneo` ahora ACREDITA LOS LUKAS a los ganadores + crea TransaccionLukas `PREMIO_TORNEO` en transacción atómica — antes solo seteaba `Ticket.premioLukas` sin mover el balance; recalcula puntos ANTES de distribuir premios para arreglar tickets con puntos stale del motor viejo; idempotencia: early-return si torneo ya FINALIZADO; endpoint admin `POST /api/v1/admin/torneos/:id/reconciliar` (+ `reconciliarTorneoFinalizado`) para reparar torneos FINALIZADOS que quedaron sin crédito — re-recalcula, re-distribuye, acredita solo el delta vs lo ya acreditado (idempotente); `/matches` y `/` ahora `force-dynamic` — antes el widget "🔴 En vivo ahora" del sidebar quedaba pegado con datos stale cuando un partido terminaba o uno nuevo entraba en vivo; copy del widget "🏆 Cómo se pagan los premios" simplificado: "En caso de empate, el premio se reparte en partes iguales entre los jugadores empatados")
