...
| **N** — Home + Socios + Socios Hub + Reviews y Guías + Perfiles ⏳ | Usuario | `docs/plan-trabajo-claude-code-v3.2.md` § Lote N | Home con 3 estados consolidados, /socios + /socios-hub rebrand sin webinars, /reviews-y-guias unificado con tabs, /perfil con cuenta inline, /jugador/[username] perfil público. **Aplicar el enfoque del Lote Q (fidelidad 1:1 mockup)** — el CSS necesario YA está en `mockup-styles.css` (Lote R), las vistas solo portan HTML del mockup a JSX. |
...
**Ruta crítica v3.2:** **K ✅** → **L ✅** → **M ✅** → **Q ✅** → **R ✅** → **S ✅** → N → O → P. K es prerequisito de todo (schema, URLs, paywall config, AuthGate). L es prerequisito de M, N, O, P (datos del motor alimentan vistas). M deja productivas las dos vistas centrales del usuario (Las Fijas + Liga); Q reescribe esas 4 vistas con fidelidad 1:1 al mockup HTML; **R completa la portación del CSS del mockup — `mockup-styles.css` ahora cubre TODAS las vistas del mockup (públicas + admin), los lotes N + O + P solo portan HTML→JSX sin escribir CSS nuevo**; **S porta los 5 componentes globales (NavBar, NavLinks, UserMenu, Footer, BottomNav) con HTML literal del mockup, dejando shell común para todos los layouts**; N completa Home + Socios + Reviews + Perfiles aplicando el mismo enfoque; O y P aterrizan admin operación + analítica.
