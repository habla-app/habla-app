...
> Última actualización: 19 de Abril 2026 (Hotfix #8 Ítem 3 — corrección del desfase del reloj del `LiveHero`: el PO reportó que el hero mostraba minuto 59' cuando el partido real iba 54'. Root cause: la fórmula del refactor anterior `max(elapsed_server, mFromKickoff)` en 1H pisaba al server con la heurística cuando esta adelantaba — era el bug OPUESTO al que pretendía resolver. Fix: regla unificada para 1H/2H/ET — si el server reporta `elapsed`, ESE es la fuente de verdad sin `max` con nada más; proyectamos localmente como `elapsed + floor((now - elapsedAnchorAt)/60000)` con cap por status. La heurística de `fechaInicio` queda reducida: solo se activa cuando `elapsed === null` (primer render pre-poller, cache vacío post-restart) y solo para 1H; para 2H/ET sin elapsed delegamos al mapper que devuelve "2T"/"Prórroga". Resto del Hotfix #8 preservado: auto-reconciliación defensiva en `finalizarTorneo` para torneos FINALIZADOS sin crédito, helper `detectarCreditoIncompleto`, payload con `statusShort` aditivo, `LiveMatchTab.fechaInicio` como input del fallback)
...
**Caso del bug del PO resuelto:** con el partido en 2H real al minuto 54, el server reporta `statusShort="2H", elapsed=54`. Antes: la rama 1H del hook nunca se activaba para este partido (statusShort era 2H), pero el primer fix tenía lógicas inconsistentes entre 1H (con max) y 2H (sin max). Ahora: rama única server-anclada. Si `elapsed=54` y delta=5s desde recepción, resultado = 54'. Si delta=65s, resultado = 55'. Nunca más desfase de 5 min.

**Tests nuevos:** `tests/use-minuto-en-vivo.test.ts` agrega `BUG REPRO PO: partido real minuto 54 + heurística desalineada → muestra 54, no 59` (2H branch) + reescribe los tests del 1H para validar que `elapsed` del server pisa siempre (1H elapsed=20 hace 60s → 21' aunque la heurística diría 60'). Total: 572 passing.
...
- **Minuto del partido en vivo se deriva con `useMinutoEnVivo` — server GANA sobre heurística cuando hay `elapsed`:** el hook `apps/web/hooks/useMinutoEnVivo.ts` es el único punto donde la UI decide qué label mostrar para el minuto en el `LiveHero`. Toma `{ fechaInicio, statusShort, elapsed }` y devuelve el string. Regla unificada:
  - **Fases avanzables con `elapsed !== null` (1H/2H/ET)**: el server reporta el minuto, proyectamos localmente sumando el delta desde que lo recibimos (`elapsedAnchorAt`, `useRef<number>` que resetea al cambiar `elapsed` o `statusShort`). Fórmula: `final = elapsed >= cap ? elapsed : min(cap, elapsed + delta)` donde `delta = floor((now - elapsedAnchorAt)/60000)` y `cap` = 45 (1H) / 90 (2H) / 120 (ET). Si el server ya reporta `elapsed >= cap` (descuento largo del PT, prórroga extendida), respetamos el server sin proyectar. **La heurística NO se mezcla con el server — si hay elapsed, el server es la verdad.**
  - **Fases fijas** (`HT`, `FT`, `AET`, `PEN`, `NS`, `TBD`, `PST`, `SUSP`, `INT`, `BT`, `P`, `CANC`, `ABD`, `AWD`, `WO`): delega al mapper `formatMinutoLabel` sin interval (cero CPU cuando el partido no avanza).
  - **Fase 1H con `elapsed === null` (cache vacío)**: heurística basada en `fechaInicio`. `minuto = max(1, min(45, floor((now - fechaInicio)/60000) + 1))`. Se activa solo hasta el primer tick del poller (≤30s post-boot); en cuanto llegue `elapsed`, el hook cambia a server-anclado.
  - **Fases 2H/ET con `elapsed === null`**: delega al mapper (devuelve "2T" / "Prórroga"). NO aplicamos heurística de `fechaInicio` porque el descanso del HT es variable (10-25 min) y no podemos derivar el minuto del 2T del kickoff sin ancla del server.
  - **Sin `statusShort`**: heurística standalone — `<0 min → "Por empezar"`, `0-45 → "{n}'"`, `45-60 → "ENT"`, `60-105 → "{n-15}'"` (asumiendo HT estándar), `>105 → "—"`. Se corrige cuando llegue el primer tick.

  Prohibido renderizar el minuto con `renderMinutoLabel` directo o con lógica inline — siempre vía el hook. Prohibido usar `max(elapsed_server, heuristica)` o cualquier fórmula que mezcle ambos — el server es la fuente de verdad absoluta cuando está disponible. Test de regresión en `tests/use-minuto-en-vivo.test.ts` verifica el caso del bug (`BUG REPRO PO: partido real minuto 54 + heurística desalineada → muestra 54`). Razón: el Ítem 3 post Hotfix #8 — la fórmula previa `max(elapsed, mFromKickoff)` pisaba el server con la heurística cuando esta adelantaba, produciendo un desfase constante de +N minutos respecto al reloj real del partido.
