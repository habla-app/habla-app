...
**Ruta crítica v3.2:** **K ✅** → **L ✅** → **M ✅** → **Q ✅** → N → O → P. K es prerequisito de todo (schema, URLs, paywall config, AuthGate). L es prerequisito de M, N, O, P (datos del motor alimentan vistas). M deja productivas las dos vistas centrales del usuario (Las Fijas + Liga); **Q reescribe esas 4 vistas con fidelidad 1:1 al mockup HTML — `mockup-styles.css` queda como base CSS para los lotes siguientes**; N completa Home + Socios + Reviews + Perfiles aplicando el mismo enfoque; O y P aterrizan admin operación + analítica.

## Enfoque "fidelidad 1:1 mockup" (Lote Q en adelante)

Desde el Lote Q el enfoque de implementación de vistas usuario cambió:
...
Procedimiento por vista para los lotes restantes (N, y eventualmente refresh de admin):
...
3. Si faltan clases CSS, agregarlas a `apps/web/app/mockup-styles.css` (las variables `:root` ya viven en `globals.css`).
